"""Isolated target-only search worker; round-level selection is separate.

Distinct branch training streams, aligned endpoint streams and fresh optimizer.
This module does not select a checkpoint or run a completion batch.
"""
import gc
import json
import os
...
from verge_search_worker_protocol import validate_prepared
...
def main():
    ...
    index = 0  # One isolated worker for one of four round-level branches.
    ...
    with VLLMServerPool(gpus=(0,), base_port=port) as urls:
        ...
        result.update(training=state, binary_rewards_only=all(s["reward_mode"] == "binary" for s in branch["stages"]),
            target_only_search=True, search_branch_index=cfg["search_branch_index"], search_round=cfg["search_round"],
            training_random_stream_id=stream, endpoint_random_stream_id=cfg["random_stream_id"],
            search_comparison_complete=False, fresh_optimizer=True, replay_loss=0.0,
