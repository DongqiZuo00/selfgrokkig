"""Bounded real Ministral/initial Solver smoke; never reads sealed model results.

Four train prompts x eight independent samples x <=512 tokens. An actual mixed
binary reward group can cause one update per group; all-equal groups skip.
...
def main():
    ...
    manifests = {str(p): digest(p) for p in [MODEL / "config.json", MODEL / "tokenizer_config.json",
                 ...
                 ADAPTER / "state.pt", Path(__file__), HERE / "runtime/solver_update.py",
    ...
    journal.append("preflight", {"model": str(MODEL), "adapter": str(ADAPTER), "sha256": manifests,
                   ...
                   "max_generated_tokens": 4 * 8 * 512, "budget_b200_hours_ceiling": 1 / 3,
                   "binary_reward_only": True, "beta": 0, "weight_decay": 0,
                   "heldout_or_test_model_results_read": False, "mode": "pipeline_smoke",
    ...
    rows = [json.loads(line) for line in (HERE / "benchmark/generated/target_train.jsonl").read_text().splitlines()][:4]
    if len(rows) != 4:
        raise ValueError("The bounded smoke requires four actual train instances")
    ...
    steps = total_tokens = total_successes = constant_groups = 0
    for group, row in enumerate(rows):
        ...
        batch = tokenizer([text] * 8, return_tensors="pt", add_special_tokens=False, padding=True).to("cuda")
        model.eval()
        torch.manual_seed(20260908 + group)
        journal.append("generation_attempt", {"group": group, "instance_id": row["id"], "count": 8, "max_new_tokens": 512})
        with torch.no_grad():
            sampled = model.generate(**batch, do_sample=True, temperature=0.8, top_p=0.95,
                    max_new_tokens=512, use_cache=True, pad_token_id=tokenizer.pad_token_id,
        ...
        journal.append("raw_generation", {"group": group, "instance_id": row["id"],
                       ...
                       "completions": texts, "total_generated_tokens": total_tokens,
                       "generated_eos_included": True, "post_eos_padding_excluded": True})
        if len(completions) != 8 or any(len(ids) > 512 for ids in completions) or total_tokens > 4 * 8 * 512:
            raise AssertionError("Generation violated the bounded smoke budget")
        verified = [evaluate_program(text, row["ground_truth"], create_robot_factory) for text in texts]
        ...
        journal.append("sampled_group", {"group": group, "instance_id": row["id"],
                       "prompt_token_ids": prompt_ids, "completion_token_ids": completions,
                       "completions": texts, "verification": verified, "rewards": rewards})
    ...
    summary = {"status": "complete", "mode": "real_backbone_pipeline_smoke", "groups": len(rows),
               "rollouts": len(rows) * 8, "generated_tokens": total_tokens, "binary_successes": total_successes,
               ...
               "b200_hours": (time.time() - started) / 3600, "max_gpu_memory_bytes": torch.cuda.max_memory_allocated(),
