...
def thresholds(test_count):
    if type(test_count) is not int or test_count <= 0:
        raise ValueError("Thresholds require a positive integer frozen test count")
    # A fixed four-slot sequence retains redundant checks if a row has few tests.
    return sorted([1 / test_count, 0.25, 0.5, 0.75])
...
def condition_vector(completion, row):
    from common import CREATE_ROBOT_FACTORY, extract_program, verify_full, _validate_verification_cases
    _validate_verification_cases(row["ground_truth"])
    try:
...
def profile(scored, rows):
    ...
    if not by_id or len(by_id) != len(rows):
        raise ValueError("Profile requires nonempty rows with unique instance IDs")
    if not scored:
        raise ValueError("Profile requires nonempty rollout observations")
    draws = {instance: set() for instance in by_id}
    for item in scored:
        instance, draw = str(item["instance_id"]), item["rollout_index"]
        if instance not in draws or type(draw) is not int or draw < 0:
            raise ValueError("Unknown instance or invalid rollout index")
        if draw in draws[instance]:
            raise ValueError("Duplicate instance/rollout identity would alter the denominator")
        draws[instance].add(draw)
    expected_draws = next(iter(draws.values()))
    if not expected_draws or any(indices != expected_draws for indices in draws.values()):
        raise ValueError("Every profile instance must have the same observed rollout indices")
    keyed = {}
    for item in scored:
