...
Production configuration remains 524288; no model weights, GPUs or Slurm used.
"""
import ast
import copy
import gc
...
from verge_search_worker_protocol import worker_prepared,validate_prepared
from verge_search_events import first_training_event,final_event
...
class RunnerTests(unittest.TestCase):
    def execute(self,exp,*,mixed=False,interrupt=False):
        ...
        cfg.update(train_tokens_per_branch=16,token_update_threshold=8,
                   search_phases=[{'kind':'target','reward_mode':'binary','tokens':16}])
        ...
        atomic_json(root/'round_frozen.json',worker_prepared(cfg,initial))
        saved_paths={};called=[];fault={'pending':interrupt}

        def evaluation(directory,kind,prompts,n):
            seed=endpoint_request_seed(cfg,kind)
            request={'prompt':[[0]]*prompts,'n':n,'seed':seed,'max_tokens':2048,'temperature':1.,'top_p':1.,'top_k':-1}
        ...
        class Client:
            ...
            def score_rows(self,rows,n,seed,path,**kwargs):
                ...
                seed=stable_int(seed,kwargs.get('sampling_seed_key') or 'verge_v2')
        ...
        tree=ast.parse((EXP/'src/verge_search_train.py').read_text())
        ...
        exec(code,env)
        with patch('verge_search_events.BUDGET',16),patch('verge_search_verify.BUDGET',16),redirect_stdout(io.StringIO()):
            if interrupt:
    ...
    def test_all_zero_target_stalls_and_is_censored(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(result['search_event']['right_censored'])
    ...
    def test_real_runner_keeps_training_event_out_of_endpoint_rate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result['search_event']['observation_loss_tokens'],8)
    ...
    def test_post_commit_resume_matches_uninterrupted(self):
        with tempfile.TemporaryDirectory() as a,tempfile.TemporaryDirectory() as b:
            ...
            self.assertEqual(actual['search_event'],expected['search_event'])
            self.assertEqual(len(calls),2)
