"""Integrated repaired Solver branch; never changes an old branch checkpoint."""
import gc
import json
import os
import time
...
from common import EXP_ROOT, read_json, atomic_json, group_advantages, stable_int
...
def main():
    ...
    if (output / "complete.json").exists():
        print(json.dumps(read_json(output / "complete.json")["training"]))
        return
    ...
    with VLLMServerPool(gpus=(0,), base_port=port) as urls:
        ...
        for phase, stage in enumerate(branch["stages"]):
            while state["used_by_stage"][phase] < stage["tokens"]:
                ...
                while used < min(cfg["token_update_threshold"], remaining):
                    ...
                    items.extend(batch)
                    advantages.extend(group_advantages(rewards, 8))
        ...
        result.update(training=state, binary_rewards_only=True, fresh_optimizer=True, replay_loss=0.0,
