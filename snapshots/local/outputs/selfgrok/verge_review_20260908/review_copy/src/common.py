...
def _validate_verification_cases(test_cases):
    # A missing test/output is a broken protocol, never a successful program.
    if not test_cases:
        raise ValueError("Verification requires a nonempty frozen test set")
    for case in test_cases:
        if not isinstance(case, dict):
            raise ValueError("Verification cases must be objects")
        if case.get("check_output", True) and not isinstance(case.get("expected_output"), str):
            raise ValueError("Output-checking cases require an explicit expected_output string")
...
def verify_full(completion: str, test_cases: list[dict[str, Any]]) -> Verification:
    ...
    _validate_verification_cases(test_cases)
