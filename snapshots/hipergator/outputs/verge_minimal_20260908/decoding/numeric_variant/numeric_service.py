"""Independent numeric-spelling CPU worker; frozen v1 files are not modified.
...
from numeric_grammar import PROGRAM_PREFIX, build_grammar, grammar_digest, parent_grammar_digest, POLICY_ID
...
def main():
    ...
    if args.json_schema is not None or args.prefix not in (None, PROGRAM_PREFIX):
        raise ValueError("Numeric variant supports only its fixed DSL prefix, not JSON or a changed prefix")
    ...
    metadata = {"status": "ready", "xgrammar_version": importlib.metadata.version("xgrammar"),
                ...
                "grammar_sha256": grammar_sha256, "max_nodes": args.max_nodes if mode == "solver_dsl_numeric" else None,
                "mode": mode, "consumed_prefix": prefix,
                "decoding_policy": POLICY_ID, "identifier_spelling": "0..K-1",
                "parent_grammar_sha256": parent_grammar_digest(args.max_nodes),
